...
- The web tier holds a **read-only** GitHub App credential. Every write to
  GitHub happens in a separate, short-lived Job that mounts a **different**
  App's credential. Compromising the web tier does not grant the ability to
  mutate an organization.
- Scheduled, unattended runs can only ever **remove** organization members.
  Additions require a signed-in operator. This asymmetry is encoded in the
  rendered reconciler config, not in a command-line flag.
